...
const sections = gsap.utils.toArray(".slide"); // array contenente le section con classe .slide
const images = gsap.utils.toArray(".image").reverse();
const slideImages = gsap.utils.toArray(".slide__img");
const outerWrappers = gsap.utils.toArray(".slide__outer");
const innerWrappers = gsap.utils.toArray(".slide__inner");
const count = document.querySelector(".count");
const wrap = gsap.utils.wrap(0, sections.length); //da un range da 0 alla lunghezza di sections cioè dell'array contenente le sezioni con la classe .slide
let animating;
let currentIndex = 0;
...
function gotoSection(index, direction) {
 animating = true;
 index = wrap(index); // seleziona la sezione dall'array delle sezioni (wrap), con l'indice passato index, vedi sopra
