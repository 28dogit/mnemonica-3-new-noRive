...
// Riferimento al container per il composable useSwipeNavigation
const sectionsWrapperRef = ref(null);
...
onMounted(() => {
  ...
  nextTick(async () => {
    ...
    // Inizializza il composable useSwipeNavigation
    const {
      isAnimating,
      currentStep,
      goToNextStep,
      ...
      maxSteps: 3, // Numero di sezioni fisse (hero, phases, modules)
      initialStep: 0,
      touchThreshold: 20,
      scrollThreshold: 8,
      scrollCooldown: 400,
      ...
      onStepChange: (step) => {
        // Quando cambia lo step, riproduci o inverti la timeline in base alla direzione
      ...
      onAnimationComplete: () => {
        // Questo callback viene chiamato quando l'animazione di transizione è completata
        console.log("Transizione completata, step corrente:", currentStep.value);
      }
    });

    // Collega gli eventi al wrapper delle sezioni fisse
    sectionsWrapperRef.value = document.getElementById('sectionsWrapper');
    if (sectionsWrapperRef.value) {
      bindEvents(sectionsWrapperRef.value);
    }
