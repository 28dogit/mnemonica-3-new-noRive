    ...
    if (!checkNofixedSection()) {
      ...
      if (nofixedSection) {
        $gsap.to(nofixedSection, {
          ...
          onComplete: () => {
            // Avvia l'animazione della timeline
            sectionsTLRef.value.tweenTo("End-phases");
          },
        });
      }
    } else {
    ...
    if (!checkNofixedSection()) {
      ...
      if (nofixedSection) {
        $gsap.to(nofixedSection, {
          ...
          onComplete: () => {
            // Avvia l'animazione della timeline
            sectionsTLRef.value.tweenTo("Start-modules-pause");
          },
        });
      }
    } else {
...
onMounted(() => {
  ...
  nextTick(async () => {
    ...
    //SECTION - animejs timeline

    // const animeTL = createTimeline({ defaults: { duration: 750 } });
    // animeTL
    //   .label("start")
    //   .add(".square", { x: "15rem" }, 500)
    //   .add(".circle", { x: "15rem" }, "start")
    //   .add(".triangle", { x: "15rem", rotate: "1turn" }, "<-=500");
    utils.set(".txt-link", { filter: "blur(5px)", scale: 0.5 });
    animate(".txt-link", { scale: 1, filter: "blur(0px)", ease: "outExpo" });
