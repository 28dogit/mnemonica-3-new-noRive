<template>
    ...
    >
      ...
      <div class="absolute bottom-10 left-0 right-0 text-center text-gray-600">
        {{
          isClient
            ? isTouchDevice
              ? "Swipe verticale per cambiare quadrato"
  ...
  <!-- per integrare i pallocchi per il passaggio diretto da uno step ad un altro vedere la pagina swipe-navigation-demo.vue -->
</template>
<script setup>
...
const playAnimation = () => {
  ...
  $gsap.set([square1.value, square2.value, square3.value], { opacity: 0 });
...
onMounted(() => {
  const container = animationContainer.value; //potrebbe essere la parte fixedSections
  ...
  const { $gsap } = useNuxtApp();
  //$gsap.set("body", { overflow: "hidden" }); // blocco lo scroll per poter testare le gesture

  // Mostra il primo quadrato all'avvio
  $gsap.set([square2.value, square3.value], { opacity: 0 });
  $gsap.set(square1.value, { opacity: 1 });
});
</script>
